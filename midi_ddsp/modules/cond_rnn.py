...
class TwoLayerCondAutoregRNN(tfk.Model):
  ...
  """
    ...
    def autoregressive(self, cond, training=False, display_progressbar=False):
        ...
        #overall_outputs = []
        #overall_outputs = tf.TensorArray(cond_encode.dtype, size=length, dynamic_size=False, clear_after_read=False)
        overall_outputs_shapes = {"f0_midi_dv_logits": (3, 1, 201), "f0_midi_dv_onehot": (3, 1, 201), "curr_out": (3, 1, 201), }
        ...
        if False:
            for i in tf.range(length):
                ...
                #print(f"i={i}")
                #print(f"tf.shape(i)={tf.shape(i)}")
                curr_cond = cond_encode[:, i, :][:, tf.newaxis, :]
                #print(f"curr_cond={curr_cond}")
                prev_out = self.encode_out(prev_out, training=training)
                curr_out, curr_states = self._one_step(curr_cond, prev_out, prev_states, training=training)
                curr_out = self.sample_out(curr_out, training=training)
                ...
                #overall_outputs = overall_outputs.write(index=i, value=curr_out)
                prev_out, prev_states = curr_out['curr_out'], curr_states

        #outputs = {}
        #for k in curr_out.keys():
        #    outputs[k] = tf.concat([overall_outputs.read(j)[k] for j in tf.range(overall_outputs.size())], 1)
        #outputs = {key: overall_outputs_for_key.stack() for key, overall_outputs_for_key in overall_outputs_per_key.items()}
        ...
        #outputs = {key: tf.foldl(fn=lambda accum, x: tf.concat((accum, x), axis=1), elems=overall_outputs_for_key.stack())
        #           for key, overall_outputs_for_key in overall_outputs_per_key.items()}

        print(f"overall_outputs_per_key={overall_outputs_per_key}")
        print(f"overall_outputs_per_key['f0_midi_dv_logits'].read(0)={overall_outputs_per_key['f0_midi_dv_logits'].read(0)}")
        print(f"overall_outputs_per_key['f0_midi_dv_logits'].stack()={overall_outputs_per_key['f0_midi_dv_logits'].stack()}")
