...
class MelF0LDEncoder(tfk.Model):
...
class F0LDEncoder(tfk.Model):
  ...
  def __init__(self):
    ...
    self.instrument_emb = tfkl.Embedding(NUM_INST, 128)
    self.rnn = tfkl.GRU(units=self.nhid, return_sequences=True, dropout=0.2)

  def call(self, inputs, training=False):
    z_f0 = self.f0_fc(ddsp.core.hz_to_midi(inputs['f0_hz']) / F0_RANGE)
    z_ld = self.ld_fc(inputs['loudness_db'] / DB_RANGE)
    instrument_z = tf.tile(self.instrument_emb(inputs['instrument_id'])[:, tf.newaxis, :], [1, z_ld.shape[1], 1])
...
class FCStackHarmonicDecoder(tfk.Model):
